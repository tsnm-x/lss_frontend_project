...
const ProfileCompareBar = (props) => {
	...
	return (
        <div className=" flex ">
            {/* profiles  */}
            <div className=" flex items-center row-start-1 col-start-1 w-[376px] pl-[40px] ">
                {profileData.map((profile, index) => {
                    return <Profile key={index} {...profile} />;
                })}
            </div>
            {/* center batch bar  */}
            <div className="  flex justify-center w-[600px] h-[40px] ">
                <DrakeBatches
            ...
            {/* right side bans  */}
            <div className=" flex items-center justify-end  row-start-1 col-start-1 w-[376px] pr-[20px] ">
                {/*  left	 */}
                <div className=" flex items-center ">
                    {redBans?.bans?.map((ban, index) => {
                        return (
                            <div
                                key={index}
                                className={` w-[30px] h-[30px] relative rounded-full -ml-[6px] first:ml-0  border border-[#D55460]`}
                            >
                                {ready && selectChampName(ban?.championId) && (
                                    <div
                ...
                {/* center  */}
                <h5 className=" sf-bold-14 text-white mx-[15px] ">Bans</h5>
                {/*  right	 */}
                <div className=" flex items-center ">
                    {blueBans?.bans?.map((ban, index) => {
                        return (
                            <div
                                key={index}
                                className={` w-[30px] h-[30px] relative rounded-full -ml-[6px] first:ml-0 border border-[#198cff] `}
                            >
                                {ready && selectChampName(ban?.championId) && (
                                    <div
                                        className="rounded-full "
