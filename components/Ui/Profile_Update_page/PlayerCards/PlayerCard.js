...
import GameSim from "../../Game_Simulation/GameSim";
...
const PlayerCards = (props) => {
	...
	const [matchByTimeline, setMatchByTimeline] = useState();
				...
				<div className="relative rounded-bl-[35px]  ">
					<div className="relative flex flex-col bg-full-dark rounded-[24px] w-[97%] ">
						...
						<div className={`flex ${cardDetailsExpand && "mb-6"}`}>
							{/* profile section  */}
							<div
								className={` relative ${cardDetailsExpand && "w-full"} ${
									classes.card
								}`}
							>
								<div
									className={` ${"bg-" + props.color} ${classes.indicator}`}
								></div>
								<div
								...
								>
									...
									<div>
										...
										<div className=" flex font-sf-pro text-[3px] leading-[3px] mb-[6px] ml-[7px]  ">
											...
											<p className=" text-[#8D919F]">{props.gameStartDate}</p>
										</div>
										{/* profile  */}
